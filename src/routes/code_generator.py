...
from src.middleware.validation import validate_request, TaskBasedRequestSchema

# Create blueprint
code_generator_bp = Blueprint('code_generator', __name__)

def _process_task_in_background(data: Dict[str, Any], cache_service):
    """
    ...
    try:
        ...
        # Initialize services (needed for both cached and non-cached paths)
        evaluation_service = EvaluationService()
        
        # Check cache first
        cache_key = f"{task_id}_round_{round_num}"
        cached_data = cache_service.get(cache_key)
        
        if cached_data:
            logger.info(f"Using cached data for task: {task_id} (Round {round_num})")
            all_files = cached_data.get('files', {})
            metadata = cached_data.get('metadata', {})
            repo_name = cached_data.get('repo_name', evaluation_service.generate_repository_name(task_id, round_num))
        else:
            logger.info(f"Generating new content for task: {task_id} (Round {round_num})")
           ...
           # test_cases = evaluation_service.convert_checks_to_tests(checks)
            ...
            logger.info(f"Cached data for task: {cache_key}")
...
def generate_and_deploy_task(data: Dict[str, Any]):
    ...
    try:
        ...
        background_thread = threading.Thread(
            ...
            args=(data, current_app.cache_service),
